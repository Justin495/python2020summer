...
class Exercise9 ( wx.Frame ):

	def __init__( self, parent ):
		wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = wx.EmptyString, pos = wx.DefaultPosition, size = wx.Size( 1050,577 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
	...
	def downloadData( self, event ):
		...
		data_flow = pd.read_csv(url, comment='#', header=0, sep='\t')[1:].apply(lambda x: pd.to_numeric(x, errors='ignore') if x.name.endswith('_va') else x, axis=0)
		print(data_flow.set_index('datetime').columns)
        
		pd.to_numeric(data_flow.set_index('datetime').iloc[:, 2]).plot(ax=self.axes)
		self.canvas = FigureCanvas(self.m_panel1, -1, self.m_panel1.figure)

	def changeFreq( self, event ):
		self.axes.clear()
		rID = even.GetInt()
		df =  pd.to_numeric(data_flow.set_index('datetime').iloc[:, 2])
		...
		self.canvas = Figure.Canvas(self.m_panel1, -1, self.m_panel1.figure)
		print('ratio is {}'.format(even.GetInt()))
